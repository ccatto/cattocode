...
const GIT_GATEWAY_STATUS_ENDPOINT = "".concat(STATUS_PAGE, "/api/v2/components.json");
...
const apiGet = async path => {
  ...
  const response = await fetch("".concat(apiRoot, "/").concat(path)).then(res => res.json());
  return response;
};
  ...
  authenticate(credentials) {
    ...
    this.tokenPromise = async () => {
      try {
        ...
        throw new _netlifyCmsLibUtil.AccessTokenError("Failed getting access token: ".concat(error.message));
      }
    };
  ...
  async getDeployPreview(collection, slug) {
    ...
    if (!preview) {
      try {
        ...
        const deploys = await apiGet("".concat(site.id, "/deploys?per_page=100"));
