...
class API {
  ...
  async editorialWorkflowGit(files, entry, mediaFilesList, options) {
    ...
    if (!unpublished) {
      ...
      const diffs = diffFiles.map(diffFromFile); // mark media files to remove
  ...
  async backupBranch(branchName) {
    try {
      ...
      await this.createBranch(existingBranch.name.replace(new RegExp("".concat(_netlifyCmsLibUtil.CMS_BRANCH_PREFIX, "/")), "".concat(_netlifyCmsLibUtil.CMS_BRANCH_PREFIX, "_").concat(Date.now(), "/")), existingBranch.commit.sha);
    } catch (e) {
      console.warn(e);
    }
  }
  ...
  async createBranch(branchName, sha) {
    try {
      ...
      } else if (message === 'Reference already exists' && branchName.startsWith("".concat(_netlifyCmsLibUtil.CMS_BRANCH_PREFIX, "/"))) {
        try {
  ...
  async uploadBlob(item) {
    ...
    const response = await this.request("".concat(this.repoURL, "/git/blobs"), {
      method: 'POST',
      body: JSON.stringify({
