...
exports.default = exports.getFolderFiles = void 0;
...
const getFolderFiles = (tree, folder, extension, depth, files = [], path = folder) => {
  if (depth <= 0) {
    return files;
  }
  ...
  Object.keys(tree[folder] || {}).forEach(key => {
    if ((0, _path.extname)(key)) {
      ...
      if (!extension || key.endsWith(".".concat(extension))) {
        files.unshift({
      ...
      return getFolderFiles(subTree, key, extension, depth - 1, files, "".concat(path, "/").concat(key));
    }
  });
  return files;
};
...
class TestBackend {
  ...
  unpublishedEntry({
    ...
    const entry = window.repoFilesUnpublished["".concat(collection, "/").concat(slug)];
  ...
  deleteUnpublishedEntry(collection, slug) {
    delete window.repoFilesUnpublished["".concat(collection, "/").concat(slug)];
    return Promise.resolve();
  }
  ...
  async persistEntry({
    ...
    if (options.useWorkflow) {
      const key = "".concat(options.collectionName, "/").concat(slug);
      ...
      this.addOrUpdateUnpublishedEntry(key, path, newPath, raw, assetProxies, slug, options.collectionName, status);
      return Promise.resolve();
    }
  ...
  updateUnpublishedEntryStatus(collection, slug, newStatus) {
    window.repoFilesUnpublished["".concat(collection, "/").concat(slug)].status = newStatus;
    return Promise.resolve();
  }

  publishUnpublishedEntry(collection, slug) {
    const key = "".concat(collection, "/").concat(slug);
